...
public:
	...
	basic_ssl_api(ApiTraits traits)
	  ...
	  , basic_ssl_constants<ApiTraits>{*static_cast<ApiTraits*>(this),
	...
	auto data_digest(memory::const_block data,
	  memory::block dst,
	  message_digest_type mdtype) const noexcept -> memory::block;

	template <typename OptMdt>
	auto do_data_digest(memory::const_block data,
	  memory::block dst,
	...
	auto sign_data_digest(memory::const_block data,
	  memory::block dst,
	  message_digest_type mdtype,
	  pkey pky) const noexcept -> memory::block;

	auto verify_data_digest(memory::const_block data,
