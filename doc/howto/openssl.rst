...
A new SoftHSM security token, with the label ``eagine`` in the slot 0, can be
created by doing:
...
Basic workflows with ZymKey engine
==================================

Zymbit ZYMKEY is a hardware crypto module for RPi.

Download required packages
--------------------------

The Raspbian packages can be installed with:

::

  curl -G https://s3.amazonaws.com/zk-sw-repo/install_zk_sw.sh |\
    sudo bash

Check the SSL engine
--------------------

After the installation, the OpenSSL engine's functionality can be checked
...
A new Zymkey hardware token, with the label ``eagine`` in the slot 0,
can be created by doing:

::

  sudo -g zk_pkcs11 zk_pkcs11-util \
    --init-token \
    --slot 0 \
    --label eagine
...
This command can be used to create a private/public key pair within the Zymkey
security module in the token (slot) with the label ``eagine``:
...
The created objects in the token with label ``eagine`` can be listed with:

::

  pkcs11-tool \
    --module /usr/lib/libzk_pkcs11.so -l \
    --token eagine \
    --list-objects
...
To generate a self-signed root CA certificate for the key pair (identified as
``eagine-ca``) in the Zymkey token with label ``eagine``, execute this command:
